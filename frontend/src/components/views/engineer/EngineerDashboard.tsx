...
export const EngineerDashboard: React.FC<EngineerDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'investigation' | 'command' | 'quality' | 'flow' | 'rootcause' | 'simulation' | 'recommendations' | 'incidents'
  >('investigation');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-white flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 font-sans">
            Real-time multivariate telemetry, 3D kinematic twin, Bayesian causal DAG, and counterfactual simulation.
          </p>
        </div>

        {/* Engineering Simulation Triggers */}
      ...
      <div className="flex items-center gap-1 border-b border-slate-200 overflow-x-auto pb-px">
        {[
          ...
          { id: 'flow', label: 'Production Flow (Queues)', icon: Activity },
          { id: 'rootcause', label: 'Root Cause (Bayesian DAG)', icon: Cpu },
          { id: 'simulation', label: 'Simulation Lab', icon: Sliders },
          { id: 'recommendations', label: 'AI Interventions', icon: Sparkles },
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2.5 text-xs font-bold uppercase font-mono tracking-wider border-b-2 transition-all flex items-center gap-1.5 whitespace-nowrap ${
                isActive
      ...
      <div>
        ...
        {activeTab === 'command' && (
          <CommandCenter
            ...
            onOpenFlowView={() => setActiveTab('flow')}
        ...
        {activeTab === 'quality' && (
          <QualityView
            ...
            onOpenAimlGuide={onOpenAimlGuide}
            onNavigateToSimulation={(stationId) => {
        ...
        {activeTab === 'simulation' && (
          <SimulationLab
            onApplyRecommendation={() => setActiveTab('recommendations')}
            onOpenAimlGuide={onOpenAimlGuide}
          />
        )}
