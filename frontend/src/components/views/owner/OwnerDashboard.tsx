...
export const OwnerDashboard: React.FC<OwnerDashboardProps> = ({
  ...
  // REAL observed-simulation data (from fantom backend / Step 5-7 analytics).
  const realStations: any[] = realAnalytics?.stations ?? [];
  const realOverview: any = realAnalytics?.overview ?? null;
  const realHotspots: string[] = realOverview?.potentialBottlenecks ?? [];
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
              <div className="text-[10px] font-sans uppercase text-slate-500 font-semibold flex items-center">
                Observed Rows Analyzed
                <HelpTooltip term="health" className="ml-1" />
              </div>
              <div className="text-2xl font-black font-mono text-slate-900 mt-1">
                {realOverview ? realOverview.rowCount.toLocaleString() : '—'}
              </div>
              <div className="text-[11px] text-slate-500 mt-1 font-sans">Observed simulation runs (real dataset)</div>
            </div>
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
              ...
              <div className="text-2xl font-black font-mono text-slate-900 mt-1">
                {fmt(realOverview?.totalQueueMean)}
              </div>
              <div className="text-[11px] text-slate-500 mt-1 font-sans">parts waiting, across 8 observed stations</div>
            </div>
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
              ...
              <div className="text-2xl font-black font-mono text-amber-600 mt-1">
                {fmt(realOverview?.totalWipMean)}
              </div>
              <div className="text-[11px] text-slate-500 mt-1 font-sans">work-in-process parts (observed input)</div>
            </div>
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
              ...
              <div className="text-sm font-bold font-mono text-slate-400 mt-2">{NA}</div>
              <div className="text-[11px] text-slate-500 mt-1 font-sans">no utilization, defect, or cost fields exist in the dataset</div>
            </div>
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
              ...
              <div className="text-[11px] text-slate-500 mt-1 font-sans">
                {realHotspots.length
                  ? `${realHotspots.map((h) => h.replace('queue_', '').toUpperCase()).join(', ')} (potential bottlenecks)`
                  : 'potential bottleneck stations (real observed data)'}
              </div>
            </div>
          ...
          <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-xs space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              {stageBottlenecks.map((stage, stageIdx) => {
                const rs = realStations[stageIdx];
                return (
                <div
                  key={stage.stageId}
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="text-[10px] font-mono text-slate-500">{rs ? `${rs.id} (observed)` : 'no observed station mapped'}</div>
                  </div>
                  ...
                  <div className="mt-3 pt-2 border-t border-slate-200/80 space-y-1 text-[11px] font-sans">
                    <div className="flex justify-between">
                      ...
                      <strong className="text-slate-400 font-mono" title="No utilization/capacity fields exist in the dataset">{NA}</strong>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">
                        Time for 1 Part:
                      ...
                      <strong className="text-slate-400 font-mono" title="No cycle-time fields exist in the dataset">{NA}</strong>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">
                        Parts Waiting (mean):
                      ...
                      <strong className={rs && rs.mean > 8000 ? 'text-red-600 font-mono' : 'text-slate-800 font-mono'}>
                        {rs ? `${fmt(rs.mean)} parts` : NA}
                      </strong>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">
                      ...
                      <span className="text-slate-700 font-mono">{rs ? `${fmt(rs.p95)} parts` : NA}</span>
                    </div>
                  </div>
                </div>
                );
            ...
            <div className="p-3 bg-amber-50/70 border border-amber-200 rounded-xl text-xs text-amber-900 flex items-start gap-2">
              ...
              <div>
                <strong className="font-sans uppercase text-[11px] block font-bold">Why the line is slowed down (from real observed data):</strong>
                <span className="font-sans">
                  In the observed simulation dataset ({realOverview ? realOverview.rowCount.toLocaleString() : '605,616'} runs), the stations with the highest queue congestion are <strong>{realHotspots.length ? realHotspots.map((h) => h.replace('queue_', '')).join(' and ') : 'c1s2 and c4s3'}</strong> (potential bottlenecks, not proven root causes). Their mean queues are ~{fmt(realStations[0]?.mean)} and ~{fmt(realStations[1]?.mean)} parts respectively, holding {realStations[0] ? realStations[0].congestionSharePct?.toFixed(1) : '24.5'}% and {realStations[1] ? realStations[1].congestionSharePct?.toFixed(1) : '23.8'}% of total congestion.
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-xs space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between font-sans">
                  <div>
                    <span className="font-bold text-slate-900 block">Station c1s2 Congestion (Potential Bottleneck)</span>
                    <span className="text-slate-500">mean queue {fmt(realStations[0]?.mean)} parts; P95 {fmt(realStations[0]?.p95)}; {realStations[0]?.congestionSharePct?.toFixed(1) ?? '24.5'}% of congestion</span>
                  </div>
                  <span className="text-sm font-mono font-black text-red-600">Rank #{realStations[0]?.congestionRank ?? 1}</span>
                </div>

                <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between font-sans">
                  <div>
                    <span className="font-bold text-slate-900 block">Station c4s3 Congestion (Potential Bottleneck)</span>
                    <span className="text-slate-500">mean queue {fmt(realStations[1]?.mean)} parts; P95 {fmt(realStations[1]?.p95)}; {realStations[1]?.congestionSharePct?.toFixed(1) ?? '23.8'}% of congestion</span>
                  </div>
                  <span className="text-sm font-mono font-black text-red-600">Rank #{realStations[1]?.congestionRank ?? 2}</span>
                </div>

                <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between font-sans">
                  <div>
                    <span className="font-bold text-slate-900 block">Monetary Impact of Queues</span>
                    <span className="text-slate-500">dataset has no cost, defect, or downtime fields</span>
                  </div>
                  <span className="text-xs font-mono font-black text-slate-400">{NA}</span>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-slate-900 text-white rounded-xl flex items-center justify-between text-xs shadow-md">
                <div>
                  ...
                  <span className="text-slate-400 text-[11px] font-sans">Demo scenario economics — the observed dataset contains no cost fields, so monetary values here are from the demo configuration, not computed from real data</span>
                </div>
                <div className="text-right">
        ...
        <div className="space-y-6 animate-in fade-in">
          <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {stations.map((st) => {
                const rst = realStations.find((r) => r.id === st.realStationId);
                return (
                <div key={st.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                  ...
                  <div className="text-[10px] font-mono text-slate-500">
                    {rst
                      ? `Observed station ${rst.id} — congestion rank #${rst.congestionRank}, ${rst.congestionSharePct?.toFixed(1)}% of facility congestion`
                      : 'no corresponding station in the observed dataset'}
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-500 pt-1 font-sans">
                    <div>Mean Queue: <strong className="text-slate-800 font-mono">{rst ? `${fmt(rst.mean)} parts` : NA}</strong></div>
                    <div>Peak Queue (P95): <strong className="text-slate-800 font-mono">{rst ? `${fmt(rst.p95)} parts` : NA}</strong></div>
                    <div>Machine Busyness: <strong className="text-slate-400 font-mono">{NA}</strong></div>
                    <div>Time for 1 Part: <strong className="text-slate-400 font-mono">{NA}</strong></div>
                  </div>
                </div>
                );
              })}
