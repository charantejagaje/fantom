...
// NOTE: the JSON body parser is scoped to THIS route on purpose. A global
// express.json() would consume POST bodies before Vite's proxy middleware,
// hanging every proxied POST to /api/v1/* (backend waits for a body that was
// already read). GETs are unaffected, which is why only POSTs broke.
app.post('/api/analyze-defect', express.json({ limit: '30mb' }), async (req, res) => {
  try {
